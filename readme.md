...
On **D3D12**, optional **mesh shaders** (`DispatchMesh`) are gated by **`r_icebridgeD3D12Raster mesh`**: when the GPU reports SM 6.6 and a non-zero mesh-shader tier, triangle-list draws of **≤256 vertices** (multiple of 3) can use a mesh pipeline that reads the same upload heap vertex bytes as a raw `ByteAddressBuffer` at `t4096`. Larger batches, strips, points, lines, and ARB programs still use the vertex-shader path. Default remains **`vertex`**.
